...
PixelNode_Effect_Rainbow = require('./PixelNode_Effect.js');
...
// draw effect
PixelNode_Effect_Rainbow.prototype.draw = function() {
	...
	for (var pixel = 0; pixel < length; pixel++)
	{
		...
		for (var ring = 0; ring < this.pixelData["domePixels"]["rings"].length;ring++) {
			c = new HSVColour(pixel/length*360+this.n+offset, 100, 100).getRGB();
			this.pixelData["domePixels"]["rings"][ring][pixel] = [c.r, c.g, c.b];
			offset += 20;
		}			    
	}
