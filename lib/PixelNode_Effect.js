...
PixelNode_Effect.prototype.draw = function() {
	...
	self.options.outputs.forEach(function(output){
		output.targets.forEach(function(outputTarget) {
			...
			if (target) {
				self.drawTarget(target, output.name);
...
PixelNode_Effect.prototype.drawTarget = function(target, output) {
...
// draw rainbow
PixelNode_Effect.prototype.fillRainbow = function(target, offset) {
	var length = target.length;
	for (var pixel = 0; pixel < length; pixel++) {
		c = new HSVColour(pixel/length*360+offset, 100, 100).getRGB();
		target[pixel] = [c.r, c.g, c.b];
		//offset += 20;
	}
}
