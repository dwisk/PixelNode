...
function PixelNode_EffectManager(options) {
	...
	// init pixelNode data
	global.pixelNode.data.extend(["effects"], {
	...
	this.draw_interval = setInterval(function() {
	  self.drawEffect();
	  self.counter += 25;
	  if (global.pixelNode.data.get("effects.autoplay") && self.counter >=(self.queueEffect.duration || 10000)) {
	  	self.counter = 0;
	  	console.log("autoplay".grey)
	  	self.nextEffect();
	  }
	}, 25);  
...
PixelNode_EffectManager.prototype.drawEffect = function() {
	...
	if (global.pixelNode.data.get("effects.force_off")) {
...
PixelNode_EffectManager.prototype.setEffectByQueueId = function(id) {
	...
	global.pixelNode.data.setSilent("effects.queueId", id);
};
...
PixelNode_EffectManager.prototype.setEffectByName = function(name) {
	...
	global.pixelNode.data.set("effect", this.effect.options);
};

PixelNode_EffectManager.prototype.initObserver = function() {
	var self = this;
	global.pixelNode.data.on("changed_effects_queueId", function(paths, value) {
		self.setEffectByQueueId(global.pixelNode.data.get("effects.queueId"));	
		global.pixelNode.data.set("effects.autoplay", false);
	})
}