...
PixelNode_GameManager.prototype.pixelCache = {};
...
PixelNode_GameManager.prototype.defaultGame = null;
...
PixelNode_GameManager.prototype.lastInteraction = new Date();
...
PixelNode_GameManager.prototype.initPixelData = function() {
	...
	self.pixelCache = new cache.Cache("pixelNode_pixelData", 557056);
//	cache.clear(self.pixelCache);
	...
	self.pixelData.master = {activated:true, count:0};
	self.pixelCache.pixelData = self.pixelData;
...
PixelNode_GameManager.prototype.prevGame = function() {
	var next = this.listId - 1;
	if (next < 0) {
...
PixelNode_GameManager.prototype.setGameByListId = function(id, silent) {
	this.pixelDataOff();
...
PixelNode_GameManager.prototype.initObserver = function() {
	...
	global.pixelNode.data.on("changed_inputs_buttons_button_front_right", function(paths, value) {
		if (value) {
			self.nextGame();
			self.lastInteraction = new Date();
		}
	...
	global.pixelNode.data.on("changed_inputs_buttons_button_front_left", function(paths, value) {
		if (value) {
			self.prevGame();
			self.lastInteraction = new Date();
		}
