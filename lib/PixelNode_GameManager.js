...
PixelNode_GameManager.prototype.drawGame = function() {
	...
	if (global.config.inputMode == "server" && self.listId != 0) {
		now = new Date();
		// reset to first effect if no interaction did happen last x mins
		if (now - 1000*self.options.idletime > self.lastInteraction) {
			self.setGameByListId(0);
		}
	}
