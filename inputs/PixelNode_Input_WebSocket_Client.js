...
PixelNode_Input_WebSocket_Client.prototype.initSockets = function() {
	...
	socket.on('data_init', function (data) {
	  ...
	  global.pixelNode.data.set(['games', 'listId'], data.data.games.listId);
	  pixelNode.gameManager.setGameByListId(data.data.games.listId, true);	
	  console.log('set game to',data.data.games.listId);
